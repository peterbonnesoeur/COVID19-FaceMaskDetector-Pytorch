...
from predictor_pytorch import model_predict, MaskNet
import torch
import torchvision
...
def detect_and_predict_mask(frame, faceNet, maskNet, device, args):
	...
	for i in range(0, detections.shape[2]):
		...
		if confidence > args["confidence"]:
			...
			# the frame
			coeff = 0.13
			(startX, startY) = (max(0, int(startX- coeff*width)), max(0, int(startY-coeff*height)	))
			(endX, endY) = (min(w - 1, int(endX + coeff*width)), min(h - 1,int(endY+coeff*height)))
	...
	if len(faces) > 0:
		preds = []
		for face in faces : 
			preds.append(model_predict( maskNet, face).tolist()[0])
...
def face_detector(args):
	...
	while True:
		...
		for frame, window_name in zip(frames, window_names):
			...
			# face mask or not
			(locs, preds) = detect_and_predict_mask(frame, faceNet, maskNet, device, args)
...
if __name__ == "__main__":
	...
	ap.add_argument("-m", "--model", type=str,
					default="models_masks/mask_detector.model",
